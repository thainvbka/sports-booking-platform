...
Bảng 3.5: So sánh giải pháp thiết kế giao diện Frontend
...
## 3.3. Giao tiếp thời gian thực và Xác thực

### 3.3.1. Socket.IO
Socket.IO cung cấp giải pháp truyền tải dữ liệu hai chiều thời gian thực thông qua kết nối bền vững dựa trên giao thức WebSocket. Trong đồ án này, Socket.IO được áp dụng để giải quyết việc đồng bộ danh sách thành viên trong các kèo đấu ghép cặp và gửi thông báo tức thời tới người dùng khi có biến động về lịch đặt sân.

Bảng 3.6: So sánh giải pháp truyền thông thời gian thực

| Giải pháp | Ưu điểm | Nhược điểm |
| :--- | :--- | :--- |
| **Socket.IO** | Truyền nhận dữ liệu hai chiều với độ trễ thấp, tự động kết nối lại khi mất mạng và có cơ chế tự động chuyển đổi giao thức dự phòng. | Cần duy trì kết nối liên tục trên máy chủ và tiêu tốn nhiều tài nguyên bộ nhớ hơn so với HTTP thông thường. |
| **SSE (Server-Sent Events)** | Cấu hình đơn giản trên giao thức HTTP hiện có, phù hợp cho việc truyền dữ liệu một chiều từ máy chủ tới máy khách. | Chỉ hỗ trợ luồng dữ liệu đơn chiều từ máy chủ, không đáp ứng được các tính năng tương tác hai chiều. |
| **HTTP Polling** | Dễ lập trình, không yêu cầu thiết lập giao thức kết nối riêng biệt hay cấu hình máy chủ đặc biệt. | Gây lãng phí băng thông do máy khách phải gửi yêu cầu liên tục theo chu kỳ thời gian. |

Trong đồ án này, Socket.IO được dùng để gửi thông báo tức thời đến người chơi và chủ sân khi có thay đổi về lịch đặt hoặc kèo ghép. Cụ thể, khi có người chơi đăng ký tham gia kèo, chủ sân phê duyệt lịch đặt, hoặc đơn đặt bị hủy do quá hạn thanh toán, hệ thống sẽ gửi thông báo trực tiếp đến tài khoản người dùng liên quan. Nhờ đó, người dùng nhận được tin báo ngay trên màn hình mà không cần tải lại trang Web.

### 3.3.2. JSON Web Token
JSON Web Token là tiêu chuẩn mã hóa thông tin dạng chuỗi ký tự chứa chữ ký số nhằm xác định danh tính và quyền hạn của người dùng. Đối với hệ thống phân quyền nhiều vai trò như đồ án này, JSON Web Token đóng vai trò làm phương tiện xác thực không trạng thái giữa máy khách và máy chủ.

Bảng 3.7: So sánh giải pháp xác thực người dùng

| Giải pháp | Ưu điểm | Nhược điểm |
| :--- | :--- | :--- |
| **JSON Web Token** | Cơ chế xác thực không lưu trạng thái trên máy chủ, dễ phân tách dịch vụ và hỗ trợ phân quyền người dùng nhanh chóng. | Khó thu hồi mã xác thực trước thời điểm hết hạn tự nhiên. |
| **Session-Cookie** | Quản lý trạng thái phiên làm việc tập trung, cho phép vô hiệu hóa quyền đăng nhập của người dùng ngay lập tức. | Đòi hỏi bộ nhớ lưu trữ phiên trên máy chủ, gây khó khăn khi cần mở rộng quy mô hệ thống. |
| **OAuth 2.0** | Tăng độ an toàn nhờ tận dụng hạ tầng của bên thứ ba như Google hoặc Facebook, giúp người dùng đăng nhập nhanh. | Quy trình thiết lập phức tạp và phụ thuộc hoàn toàn vào trạng thái hoạt động của nhà cung cấp dịch vụ. |

Trong đồ án, JSON Web Token được dùng để xác thực và phân quyền cho người chơi, chủ sân và quản trị viên. Sau khi người dùng đăng nhập thành công, máy chủ cấp một chuỗi mã để máy khách đính kèm vào tiêu đề của các yêu cầu gửi lên API sau đó. Nhờ đó, máy chủ có thể kiểm tra trực tiếp thông tin người dùng từ token mà không cần truy vấn cơ sở dữ liệu ở mỗi yêu cầu.

## 3.4. Tích hợp dịch vụ ngoài và triển khai
...
Bảng 3.8: So sánh giải pháp cổng thanh toán trực tuyến
...
Bảng 3.9: So sánh giải pháp lưu trữ hình ảnh đa phương tiện
...
Bảng 3.10: So sánh giải pháp triển khai gợi ý thông minh
...
### 3.4.4. Docker & Docker Compose
Docker là công cụ tạo môi trường chạy ứng dụng độc lập dưới dạng các container. Docker Compose giúp quản lý các container này qua một tệp cấu hình duy nhất, cho phép khởi động toàn bộ hệ thống bằng một câu lệnh.

Bảng 3.11: So sánh giải pháp ảo hóa và triển khai
...
Dựa trên các tiêu chí về hiệu năng, tính nhất quán môi trường và khả năng triển khai nhanh, đồ án lựa chọn Docker kết hợp Docker Compose làm giải pháp đóng gói và triển khai hệ thống. Theo phương án này, các dịch vụ bao gồm Backend Express.js, cơ sở dữ liệu PostgreSQL, Redis và Nginx được đóng gói thành các container độc lập chạy chung một mạng nội bộ ảo. Cách làm này giúp ứng dụng hoạt động giống nhau giữa máy tính cá nhân của lập trình viên và máy chủ thực tế, hạn chế lỗi do lệch phiên bản hoặc môi trường cài đặt.


### 3.4.5. Nginx
Nginx là phần mềm máy chủ Web và proxy ngược có cơ chế hoạt động bất đồng bộ. Điều này giúp hệ thống chịu tải tốt và tiêu tốn ít RAM hơn khi có nhiều kết nối đồng thời từ người dùng.

Bảng 3.12: So sánh giải pháp máy chủ Web và proxy ngược
...
Trong đồ án, Nginx tiếp nhận các yêu cầu truy cập ban đầu để phục vụ giao diện React, đồng thời định tuyến các yêu cầu API đến máy chủ Backend chạy nội bộ. Việc này giúp giảm tải cho Backend và bảo vệ máy chủ bằng cách che giấu các cổng kết nối bên trong.
...
Bảng 4.3 chi tiết các trường thông tin của tài khoản người dùng. Trường `email` bắt buộc phải là duy nhất để tránh trùng lặp khi đăng ký, còn mật khẩu được mã hóa qua trường `password`. Các thông tin liên hệ được lưu ở `full_name`, `phone_number` và trạng thái xác thực tài khoản lưu ở `email_verified`.
...
Bảng 4.4 lưu thông tin khu phức hợp thể thao do chủ sân quản lý thông qua khóa ngoại `owner_id`. Tên và địa chỉ chi tiết được lưu ở `complex_name` và `complex_address`. Trạng thái duyệt sân được lưu ở `status`, kèm theo các ảnh chụp giấy tờ pháp lý dạng JSON ở `verification_docs` để người quản trị đối chiếu.
...
Bảng 4.5 lưu trữ lịch đặt sân của người chơi. Các trường `start_time` và `end_time` ghi lại giờ bắt đầu và kết thúc của lượt chơi để tránh trùng lịch. Trường `total_price` lưu số tiền cần trả, được liên kết với người đặt qua `player_id`, sân con qua `sub_field_id` và mã thanh toán qua `payment_id`.
...
Bảng 4.6 chi tiết cấu trúc các sân con trong khu phức hợp thông qua khóa ngoại `complex_id`. Tên sân, sức chứa và loại môn thể thao được lưu ở `sub_field_name`, `capacity` và `sport_type`. Ngoài ra, trường `embedding` lưu véc-tơ đặc trưng 8 chiều của sân đấu để phục vụ thuật toán tìm kiếm và gợi ý sân phù hợp cho người dùng.
...
Bảng 4.7 quản lý thông tin các kèo ghép do người chơi tạo ra. Mỗi kèo ghép liên kết với một đơn đặt sân qua `booking_id` và người tạo qua `creator_id`. Tiêu đề, môn chơi và trình độ yêu cầu được lưu ở `title`, `sport_type` và `skill_level`, trong khi `slots_needed` và `slots_filled` dùng để đếm số người còn thiếu.
...
Bảng 4.8 lưu thông tin phân chia doanh thu của từng đơn đặt sân để chi trả cho chủ sân. Trường `total_amount` là số tiền khách thanh toán, hệ thống sẽ giữ lại một khoản phí ở trường `platform_fee` và tính số tiền thực nhận cho chủ sân tại `payout_amount`. Trạng thái thanh toán của từng giao dịch được theo dõi ở trường `status`.
...
Bảng 4.9 gom các yêu cầu rút tiền của từng chủ sân qua `owner_id` thành các đợt quyết toán để chuyển khoản một lần. Tổng số tiền chuyển được lưu ở `total_payout` và trạng thái đợt rút lưu ở `status`. Sau khi chuyển khoản thành công, người quản trị sẽ cập nhật mã giao dịch ngân hàng vào `transaction_ref`.
...
Giao diện đối soát tài chính và phê duyệt của quản tr�### 5.2.2 Quy trình kỹ thuật và cơ chế hoạt động của đường ống gợi ý

Quy trình xử lý gợi ý sân đấu được thiết kế thành một chuỗi xử lý khép kín gồm bốn giai đoạn chính bao gồm: xây dựng đặc trưng hành vi dưới dạng véc-tơ, tìm kiếm tương đồng trên cơ sở dữ liệu, xếp hạng lại kết quả bằng mô hình ngôn ngữ lớn và lưu trữ bộ đệm để tối ưu hóa truy vấn.

Giai đoạn thứ nhất thực hiện biểu diễn các thuộc tính hành vi của người dùng và đặc trưng sân đấu dưới dạng véc-tơ 8 chiều. Véc-tơ của người dùng được xây dựng từ lịch sử tối đa 30 lượt đặt sân đã hoàn thành gần nhất, phản ánh các thói quen về môn thể thao ưa thích, khung giờ chơi thường chọn, ngày trong tuần, mức giá trung bình, khu vực địa lý, chất lượng sân đấu, tần suất đặt sân và thời gian hoạt động gần nhất. Tương tự, mỗi sân đấu trong cơ sở dữ liệu cũng được biểu diễn bằng véc-tơ đặc trưng 8 chiều tương ứng dựa trên thuộc tính môn thể thao, giờ hoạt động, biểu giá giờ cao điểm và thấp điểm, vị trí địa bàn và điểm đánh giá trung bình từ người dùng. Giá trị của tất cả các chiều trong véc-tơ được chuẩn hóa về đoạn từ 0.0 đến 1.0 để đảm bảo tính đồng nhất khi tính toán khoảng cách. Bên cạnh việc tính véc-tơ số thực, hệ thống đồng thời trích xuất hồ sơ sở thích thô của người dùng gồm tên bộ môn chơi nhiều nhất, khung giờ đặt trung bình, tỷ lệ ngày thường và cuối tuần, mức giá trung bình, khu vực quận thường chơi và điểm đánh giá trung bình của các sân đã từng trải nghiệm. Các thông số này được dịch sang tiếng Việt và kết hợp cùng thông tin thời gian thực tế tại Việt Nam theo múi giờ GMT+7 gồm giờ hiện tại, thứ trong tuần nhằm làm phong phú dữ liệu ngữ cảnh cho bước xếp hạng lại bằng mô hình ngôn ngữ.

Giai đoạn thứ hai thực hiện tìm kiếm tương đồng véc-tơ trên cơ sở dữ liệu PostgreSQL sử dụng phần mở rộng pgvector. Khi nhận được yêu cầu lấy gợi ý từ ứng dụng máy khách, hệ thống sử dụng véc-tơ đặc trưng của người chơi để tính toán khoảng cách Cosine với véc-tơ của tất cả các sân con đang hoạt động thông qua câu lệnh truy vấn SQL chứa toán tử `<=>`. Việc sử dụng khoảng cách Cosine giúp hệ thống đo lường sự tương đồng về xu hướng hành vi mà không bị ảnh hưởng bởi tần suất đặt sân tuyệt đối của người dùng. Để tăng tốc độ truy vấn, một chỉ mục kiểu IVFFlat được thiết lập trên cột chứa dữ liệu véc-tơ của bảng sân con, giúp thu hẹp phạm vi tìm kiếm và giảm độ phức tạp từ quét toàn bộ cơ sở dữ liệu xuống tìm kiếm trên các phân vùng gần nhất. Kết quả của giai đoạn này trả về danh sách 20 sân con có độ tương đồng toán học cao nhất, đồng thời tự động loại trừ các sân con mà người chơi đã đặt trong vòng 14 ngày gần nhất để đảm bảo tính đa dạng của các đề xuất.

Giai đoạn thứ ba thực hiện xếp hạng lại danh sách kết quả bằng mô hình ngôn ngữ lớn Gemini. Thay vì truyền trực tiếp các véc-tơ số thực khó suy luận cho mô hình ngôn ngữ, hệ thống gửi hồ sơ sở thích dạng văn bản tiếng Việt của người chơi cùng ngữ cảnh thời gian thực tế và thông tin chi tiết của 20 sân con ứng viên đã được lọc bớt các thuộc tính kỹ thuật dư thừa. Dựa trên các nguyên tắc thiết lập sẵn, mô hình Gemini đối chiếu mức độ tương thích về bộ môn thể thao, khoảng cách địa lý giữa các quận, biểu giá so với ngân sách của người chơi và tính phù hợp thời gian thực của khung giờ hiện tại với thói quen sinh hoạt. Từ đó, mô hình xếp hạng lại danh sách, lựa chọn ra 10 kết quả phù hợp nhất và tự động sinh lý do gợi ý ngắn gọn bằng tiếng Việt dưới 80 ký tự giải thích lý do đề xuất sân đấu đó. Để đảm bảo tính ổn định của dịch vụ, nếu mô hình ngôn ngữ gặp sự cố hoặc không phản hồi trong thời gian quy định, hệ thống sẽ tự động chuyển sang chế độ dự phòng bằng cách sử dụng trực tiếp 10 kết quả đầu tiên thu được từ bước truy vấn pgvector.

Giai đoạn thứ tư sử dụng bộ nhớ đệm Redis để tối ưu hóa hiệu năng và giảm tải cho hệ thống. Kết quả sau bước xếp hạng được lưu trữ vào Redis dưới dạng khóa giá trị với thời gian sống mặc định là 6 giờ. Khi người chơi truy cập lại ứng dụng trong khoảng thời gian này, hệ thống sẽ lấy trực tiếp kết quả từ bộ nhớ đệm mà không cần thực hiện lại các bước tính toán véc-tơ hay gọi mô hình ngôn ngữ lớn. Cơ chế làm mới bộ nhớ đệm được kích hoạt tự động bất đồng bộ ngay khi người chơi hoàn thành một giao dịch đặt sân mới hoặc gửi đánh giá chất lượng sân đấu, đảm bảo các đề xuất luôn phản ánh đúng các thay đổi hành vi gần nhất.

Đối với những người dùng mới đăng ký tài khoản hoặc có dưới 3 lượt đặt sân hoàn thành, hệ thống sẽ kích hoạt luồng xử lý dự phòng để giải quyết vấn đề khởi đầu lạnh. Trong trường hợp này, hệ thống sẽ đề xuất danh sách 10 sân đấu phổ biến nhất dựa trên việc tính toán tổng số lượt đặt sân kết hợp với điểm số đánh giá trung bình từ toàn bộ cơ sở dữ liệu người dùng. | :---: |
...
Trong các hệ thống đặt chỗ trực tuyến có lưu lượng truy cập biến động lớn theo thời gian, việc kiểm soát tranh chấp đồng thời tại các khung giờ cao điểm là một thách thức kỹ thuật phức tạp. Để duy trì tính toàn vẹn và nhất quán của dữ liệu dưới tải trọng lớn, hệ thống cần áp dụng các Giao thức kiểm soát đồng thời (Concurrency Control Protocols) `[1]`. Khi nhiều người chơi cùng thực hiện thao tác đặt chỗ cho cùng một sân đấu tại cùng một thời điểm tính bằng mili-giây, hệ thống đối mặt với hiện tượng tranh chấp tài nguyên (race condition) do bản chất bất đồng bộ của môi trường mạng. Logic nghiệp vụ đặt sân thông thường bao gồm hai bước tuần tự: kiểm tra trạng thái trống của lịch và ghi nhận đơn đặt sân mới. Nếu không có cơ chế đồng bộ hóa, các giao dịch song song thực thi dưới cấp độ cô lập mặc định (thường là Read Committed) trong các Hệ quản trị cơ sở dữ liệu quan hệ (RDBMS) có thể cùng đọc trạng thái lịch trống và đồng thời ghi nhận bản ghi đặt sân mới, dẫn đến tình trạng đặt trùng khung giờ. Sự cố này trực tiếp phá vỡ các thuộc tính ACID của giao dịch, đặc biệt là tính cô lập (Isolation) và tính nhất quán (Consistency). Bên cạnh bài toán loại trừ tương hỗ tức thời, nghiệp vụ đặt sân còn đòi hỏi cơ chế giữ chỗ tạm thời (Reservation Pattern) để bảo lưu khung giờ trong suốt tiến trình người dùng chuyển hướng sang cổng thanh toán trực tuyến. Do đó, việc thiết kế một giải pháp bảo vệ đa tầng tích hợp Khóa phân tán (Distributed Locks) ở tầng ứng dụng và cơ chế khóa dòng ở tầng cơ sở dữ liệu là điều kiện tiên quyết để đảm bảo tính an toàn cho hệ thống.

### 5.1.2 Kiến trúc ba lớp kiểm soát đồng thời và giữ chỗ

Kiến trúc ba lớp được thiết kế trên nguyên lý phòng thủ theo chiều sâu, thực hiện phân rã quyền kiểm soát tương ứng với ba lớp: lớp khóa phân tán ở tầng ứng dụng, lớp khóa bi quan ở tầng cơ sở dữ liệu, và lớp nghiệp vụ giữ chỗ tạm thời.

Lớp bảo vệ thứ nhất hoạt động ở tầng ứng dụng và sử dụng cơ chế khóa phân tán Redis làm cửa ngõ kiểm soát các yêu cầu truy cập đồng thời. Dựa trên lý thuyết về loại trừ tương hỗ (Mutual Exclusion - Mutex) trong các hệ thống phân tán `[2]`, khóa phân tán đảm bảo rằng tại một thời điểm chỉ có duy nhất một tiến trình được phép truy cập và thay đổi trạng thái của một tài nguyên dùng chung. Hệ thống xây dựng một khóa định danh duy nhất (lock key) dựa trên mã của sân con kết hợp với một chuỗi ngẫu nhiên dạng UUID. Tiến trình xử lý gửi yêu cầu thiết lập khóa nguyên tử lên bộ lưu trữ khóa-giá trị Redis bằng lệnh `SET key value NX EX 15`. Thuộc tính NX (Not Exists) đảm bảo khóa chỉ được tạo thành công nếu chưa tồn tại, đại diện cho việc yêu cầu đầu tiên giành được khóa, trong khi thuộc tính EX thiết lập thời hạn sống (TTL) của khóa là 15 giây nhằm giải phóng tài nguyên tự động và ngăn chặn tình trạng khóa chết (deadlock) nếu máy chủ ứng dụng gặp sự cố. Khi có hàng trăm yêu cầu tranh chấp cùng một khung giờ gửi lên đồng thời, chỉ có duy nhất yêu cầu chiếm được khóa thành công được đi tiếp vào luồng xử lý chính, các yêu cầu còn lại sẽ bị từ chối ngay lập tức ở tầng API để giảm tải cho cơ sở dữ liệu. Để giải phóng khóa an toàn khi hoàn tất giao dịch, hệ thống sử dụng một kịch bản Lua chạy nguyên tử (Atomic Lua Script) để kiểm tra tính khớp của chuỗi UUID ban đầu trước khi thực hiện xóa khóa, loại bỏ hoàn toàn rủi ro một tiến trình chạy chậm do độ trễ mạng tự ý xóa nhầm khóa thuộc quyền sở hữu của tiến trình khác.

Lớp bảo vệ thứ hai hoạt động tại tầng cơ sở dữ liệu PostgreSQL thông qua cơ chế khóa bi quan cấp dòng (Pessimistic Locking) `[3]`, đóng vai trò là chốt chặn cuối cùng bảo vệ tính toàn vẹn của dữ liệu. Khóa phân tán trên bộ nhớ đệm tuy có tốc độ phản hồi nhanh nhưng vẫn tồn tại xác suất rủi ro do trượt đồng hồ hệ thống hoặc mất kết nối mạng tạm thời giữa các nút. Do đó, hệ thống bao bọc toàn bộ logic kiểm tra trạng thái trống và ghi dữ liệu trong một giao dịch cơ sở dữ liệu (Database Transaction) kết hợp câu lệnh khóa dòng độc quyền (`SELECT ... FOR UPDATE`). Khi giao dịch bắt đầu, PostgreSQL sẽ phong tỏa dòng dữ liệu của sân con tương ứng, buộc các giao dịch song song khác muốn ghi nhận trên cùng dòng đó phải xếp hàng chờ cho đến khi giao dịch hiện tại hoàn tất (Commit) hoặc bị hủy bỏ (Rollback). Nhờ vậy, trạng thái trống của lịch sân được xác thực chính xác mà không chịu ảnh hưởng từ bất kỳ tiến trình đồng thời nào khác, duy trì tính cô lập (Isolation) ở mức cao nhất mà không cần nâng cấp toàn bộ hệ thống lên cấp độ cô lập Serializable vốn gây giảm hiệu năng nghiêm trọng.

Lớp bảo vệ thứ ba thực hiện cơ chế giữ chỗ tạm thời ở cấp độ logic nghiệp vụ dựa trên trạng thái đơn hàng và trường thời gian hết hạn (`expires_at`). Sau khi vượt qua hai lớp khóa kỹ thuật, đơn đặt sân được khởi tạo thành công ở trạng thái `PENDING` với thời hạn giữ chỗ là 5 phút. Trong thời gian này, khung giờ tương ứng được đánh dấu là đã có người giữ và hệ thống sẽ từ chối các yêu cầu đặt lịch khác từ phía người chơi khác. Thiết kế này tạo ra vùng đệm thời gian cần thiết để người chơi thực hiện các thao tác xác thực giao dịch trên các cổng thanh toán bên thứ ba như Stripe hay VNPay mà không lo ngại việc bị chiếm dụng lịch. Nếu quá thời hạn quy định mà hệ thống chưa nhận được thông báo xác thực thanh toán thành công thông qua cơ chế phản hồi tự động (Webhook), một tiến trình chạy ngầm (cron job) sẽ tự động chuyển trạng thái đơn đặt sân thành hủy (`CANCELED`) và giải phóng khung giờ lịch. Đối với trường hợp người chơi cố tình gửi lại yêu cầu đặt đúng khung giờ của chính đơn đặt đang chờ thanh toán còn hạn của họ, hệ thống sẽ tự động nhận diện và gia hạn thời gian giữ chỗ thay vì tạo thêm bản ghi mới.

Sự phối hợp tuần tự của ba lớp kiểm soát giúp tối ưu hóa cả hiệu năng lẫn độ an toàn cho hệ thống. Tầng ứng dụng Redis giải quyết bài toán tải lớn với chi phí tài nguyên thấp, tầng cơ sở dữ liệu PostgreSQL đảm bảo tuyệt đối tính toàn vẹn dữ liệu thông qua các ràng buộc giao dịch ACID, và lớp nghiệp vụ giải quyết bài toán giữ chỗ cho các giao dịch kéo dài. Sự bù trừ lẫn nhau giữa các lớp giúp khắc phục các điểm yếu cục bộ của từng công nghệ riêng lẻ, tạo ra một giải pháp đồng bộ và tin cậy cho bài toán đặt sân đồng thời.

Hình 5.1a và Hình 5.1b mô tả chi tiết luồng xử lý tương tác giữa các thành phần phần mềm để thực hiện cơ chế kiểm soát đồng thời ba lớp và tiến trình dọn dẹp các đơn đặt sân quá hạn.
...
skinparam SequenceLifeLineBackgroundColor #BDBDBD

title seq Luồng xử lý đặt sân đồng thời qua ba lớp
...
activate P
P -> BC : POST /bookings/:subFieldId
activate BC
BC -> BS : createBooking(playerId,\ndata, subFieldId)
activate BS
||40||

== Lớp 1: Redis Distributed Lock ==
||40||
BS -> R : acquireLock(lockKey,\nlockValue, lockTTL)
activate R
||40||
R --> BS : lockAcquired (true/false)
deactivate R
...
||40||
    ...
    BS -> PG : Check overlapping\nbookings
||40||
    PG --> BS : overlappingBooking
    
    alt overlappingBooking == null

== Lớp 3: Soft Lock (PENDING) ==
||40||
        BS -> PG : Insert PENDING & COMMIT
||40||
        PG --> BS : Success
||40||
        BS -> R : releaseLock(lockKey, lockValue)
        BS --> BC : Success (201)
        BC --> P : 201 Created (Pending)
        
    else overlappingBooking != null
||40||
        alt isOwnPending == true
            
            BS -> PG : Update expires_at & COMMIT
||40||
            PG --> BS : Success
||40||
            BS -> R : releaseLock(lockKey, lockValue)
            BS --> BC : Success (200)
            BC --> P : 200 OK
        else isOwnPending == false
            
            BS -> PG : ROLLBACK
||40||
            BS -> R : releaseLock(lockKey, lockValue)
            BS --> BC : Error (400)
            BC --> P : 400 Bad Request
        end
    end
    deactivate PG
   
else lockAcquired == false
    BS --> BC : Error (429)
    BC --> P : 429 Too Many Requests
end
...
*Hình 5.1a: Sơ đồ trình tự xử lý đặt sân đồng thời qua ba lớp bảo vệ (Tầng ứng dụng & Tầng DB)*
...
title seq Tiến trình dọn dẹp và giải phóng đặt sân quá hạn

participant "Cron Trigger\n(node-cron)" as Cron
participant "CronService" as CS
participant "BookingAddonService" as BAS
database "PostgreSQL DB" as PG
participant "NotificationService" as NS

activate Cron
Cron -> CS : cleanupExpiredBookings() (Mỗi phút)
activate CS

CS -> PG : booking.findMany(status=PENDING, expires_at < now)
activate PG
PG --> CS : expiredBookings
deactivate PG

loop for each expiredBooking
    CS -> PG : prisma.$transaction()
    activate PG
    
    CS -> PG : booking.updateMany(status=CANCELED)
    
    note over CS, BAS : Hoàn trả lại số lượng sản phẩm thuê kèm (add-ons) vào kho
    CS -> BAS : restoreAddonStockForBooking(tx, bookingId)
    activate BAS
    BAS -> PG : product.updateMany(stock = stock + quantity)
    BAS --> CS : void
    deactivate BAS
    ...
    note over CS, NS : Gửi thông báo tự động cho người chơi về việc hủy đơn
    CS -> NS : sendNotificationIfNotExists(playerId, notificationData)
    activate NS
    NS --> CS : void
    deactivate NS
end
...
*Hình 5.1b: Sơ đồ trình tự tiến trình chạy ngầm dọn dẹp và giải phóng đặt sân quá hạn*
...
Trong các ứng dụng đặt sân thể thao truyền thống, việc tìm kiếm chủ yếu phụ thuộc vào bộ lọc tĩnh như địa giới hành chính, bộ môn thể thao hoặc các mốc giờ cố định do người dùng nhập thủ công. Phương pháp này đòi hỏi người chơi phải chủ động thực hiện nhiều thao tác lặp đi lặp lại và không tận dụng được dữ liệu lịch sử để cá nhân hóa dịch vụ. Các thuật toán thống kê đơn giản chỉ đưa ra được các đề xuất mang tính đại trà dựa trên tần suất đặt sân chung, do đó không thể nhận diện được các đặc trưng hành vi phức tạp như thói quen chơi bóng vào ban đêm tại các quận lân cận của từng cá nhân. Ngoài ra, việc xây dựng hệ gợi ý còn gặp phải thách thức từ hiện tượng khởi đầu lạnh (cold start) khi người dùng mới chưa có đủ dữ liệu lịch sử để phân tích sở thích. Để giải quyết triệt để các hạn chế này, đồ án đề xuất giải pháp xây dựng hệ gợi ý dựa trên Kiến trúc gợi ý hai giai đoạn (Two-Stage Recommendation Architecture) `[4]`. Kiến trúc tiêu chuẩn này bao gồm giai đoạn một là Truy xuất (Retrieval) nhằm lọc nhanh từ cơ sở dữ liệu lớn xuống một tập nhỏ các ứng viên tiềm năng thông qua tìm kiếm tương đồng véc-tơ, và giai đoạn hai là Xếp hạng lại (Reranking) sử dụng mô hình ngôn ngữ lớn (LLM) để tối ưu hóa độ chính xác dựa trên ngữ cảnh chi tiết. Sự kết hợp giữa phần mở rộng pgvector của PostgreSQL để lọc thô ứng viên, mô hình Gemini để xếp hạng lại, và bộ nhớ đệm Redis giúp đáp ứng cả yêu cầu về chất lượng gợi ý lẫn thời gian phản hồi thực tế.

### 5.2.2 Quy trình kỹ thuật và cơ chế hoạt động của đường ống gợi ý

Quy trình gợi ý cá nhân hóa hoạt động theo nguyên lý đường ống khép kín, được phân chia thành bốn giai đoạn kế tiếp nhau nhằm tối ưu hóa độ chính xác của đề xuất và thời gian xử lý của hệ thống.

Giai đoạn thứ nhất thực hiện biểu diễn các thuộc tính hành vi của người dùng và đặc trưng sân đấu thành các điểm trong không gian véc-tơ (Vector Space Model) tám chiều [8]. Véc-tơ của người dùng được xây dựng bằng cách tổng hợp dữ liệu từ tối đa ba mươi lượt đặt sân đã hoàn thành gần nhất, phản ánh xu hướng hành vi qua tám thành phần bao gồm môn thể thao ưa thích, thời gian chơi trong ngày, ngày trong tuần, mức giá trung bình, vị trí địa lý, chất lượng sân, tần suất hoạt động và mức độ hoạt động gần nhất. Tương tự, mỗi sân đấu con trong hệ thống cũng được mô tả bằng một véc-tơ đặc trưng tương ứng dựa trên thuộc tính môn thể thao, giờ hoạt động, biểu giá, vị trí địa bàn và điểm đánh giá trung bình. Tất cả giá trị số thực trên các chiều của véc-tơ đều được chuẩn hóa tuyến tính về đoạn từ 0.0 đến 1.0 nhằm đảm bảo các thuộc tính có thang đo khác nhau đóng góp tỷ trọng đồng đều khi tính toán khoảng cách. Song song với quá trình số hóa này, hệ thống trích xuất hồ sơ sở thích thô của người dùng dưới dạng văn bản tự nhiên tiếng Việt và kết hợp với ngữ cảnh thời gian thực tại thời điểm tìm kiếm để làm phong phú thêm thông tin đầu vào phục vụ cho giai đoạn xếp hạng sau đó.

Giai đoạn thứ hai thực hiện tìm kiếm tương đồng véc-tơ trên cơ sở dữ liệu PostgreSQL thông qua phần mở rộng pgvector `[5]`. Khi nhận được yêu cầu gợi ý, hệ thống tính toán độ tương đồng Cosine (Cosine Similarity) giữa véc-tơ hành vi của người chơi và véc-tơ đặc trưng của các sân con đang hoạt động thông qua toán tử `<=>`. Về mặt toán học, độ tương đồng Cosine đo lường cosin của góc tạo bởi hai véc-tơ $\mathbf{A}$ và $\mathbf{B}$ trong không gian đa chiều, được xác định bằng tỷ số giữa tích vô hướng và tích độ dài của chúng [8]:

$$\text{Cosine Similarity} = \frac{\mathbf{A} \cdot \mathbf{B}}{|\mathbf{A}| |\mathbf{B}|}$$

Việc lựa chọn khoảng cách Cosine giúp hệ thống đánh giá sự tương quan về xu hướng sở thích dựa trên hướng của véc-tơ thay vì độ lớn tuyệt đối vốn dễ bị ảnh hưởng bởi tần suất đặt sân. Để tránh việc quét tuyến tính toàn bộ bảng dữ liệu (Flat Search) gây quá tải khi quy mô dữ liệu tăng lên, hệ thống áp dụng kỹ thuật tìm kiếm lân cận gần đúng (Approximate Nearest Neighbor - ANN) bằng cách thiết lập chỉ mục IVFFlat (Inverted File Index) trên trường chứa dữ liệu véc-tơ. Chỉ mục IVFFlat hoạt động dựa trên lý thuyết phân cụm không gian véc-tơ bằng thuật toán k-means thành các phân vùng xác định. Khi thực thi truy vấn, hệ thống chỉ so sánh véc-tơ truy vấn với các véc-tơ nằm trong các cụm có tâm gần nhất, giúp giảm độ phức tạp tính toán xuống mức dưới tuyến tính và tăng đáng kể tốc độ phản hồi. Kết quả thu được là danh sách hai mươi sân con có điểm tương đồng cao nhất, tự động loại trừ các sân mà người chơi đã đặt trong mười bốn ngày gần nhất để tăng tính đa dạng.

Giai đoạn thứ ba thực hiện xếp hạng lại danh sách hai mươi ứng viên thu được từ bước truy xuất véc-tơ bằng mô hình ngôn ngữ lớn Gemini làm bộ tái xếp hạng (LLM Reranking) `[6]`. Việc sử dụng mô hình ngôn ngữ lớn làm Reranker khai thác khả năng suy luận và học tập ngữ cảnh (in-context learning) để phân tích các yếu tố ngữ nghĩa phức tạp khó số hóa thành véc-tơ, ví dụ như sự phù hợp giữa tên địa danh (quận, huyện), khung giờ thực tế so với lịch sinh hoạt, hoặc lý giải lý do đề xuất. Backend gửi danh sách ứng viên cùng hồ sơ sở thích thô tiếng Việt của người chơi đến Gemini API. Mô hình đối chiếu thông tin để xếp hạng lại danh sách ứng viên, lựa chọn ra mười sân con phù hợp nhất và tự động sinh câu giải thích lý do đề xuất ngắn gọn dưới 80 ký tự. Để đảm bảo tính nhất quán của dữ liệu phản hồi, hệ thống áp dụng kỹ thuật ràng buộc định dạng đầu ra (Structured Outputs) theo lược đồ JSON cố định `[7]`. Trong trường hợp Gemini API gặp sự cố hoặc quá thời gian phản hồi, hệ thống tự động kích hoạt cơ chế dự phòng, lấy trực tiếp mười kết quả có độ tương đồng cao nhất từ pgvector.

Giai đoạn thứ tư sử dụng bộ nhớ đệm Redis để tối ưu hóa hiệu năng truy cập của người dùng và giảm thiểu chi phí gọi API bên ngoài. Kết quả sau khi xếp hạng được lưu trữ vào bộ nhớ đệm với thời gian sống mặc định là sáu giờ. Khi người chơi truy cập lại ứng dụng trong khoảng thời gian này, hệ thống sẽ trả về trực tiếp kết quả từ bộ nhớ đệm Redis mà không cần thực thi lại các bước tính toán véc-tơ hay gọi mô hình ngôn ngữ lớn. Cơ chế làm mới bộ nhớ đệm được cấu hình kích hoạt bất đồng bộ ngay khi người chơi hoàn thành một giao dịch đặt sân mới hoặc gửi đánh giá chất lượng dịch vụ, đảm bảo danh sách gợi ý luôn cập nhật theo hành vi mới nhất của người chơi.

Đối với những người chơi mới tạo tài khoản hoặc có dưới ba đơn đặt sân hoàn thành, hệ thống sẽ tự động kích hoạt luồng xử lý dự phòng để giải quyết vấn đề khởi đầu lạnh. Trong trường hợp này, hệ thống đề xuất danh sách mười sân đấu phổ biến nhất dựa trên tổng số lượt đặt sân kết hợp với điểm đánh giá trung bình từ toàn bộ cơ sở dữ liệu người dùng.
...
if (Đã có sẵn gợi ý?) then ([Có])
  :Lấy danh sách gợi ý sẵn có;
else ([Không])
  :Lấy lịch sử đặt sân của người chơi;
  if (Số lượt đặt sân hoàn tất?) then ([Dưới 3 lượt])
    ...
    :Xếp hạng lại bằng mô hình Gemini;
    if (Gọi Gemini API thành công?) then ([Có])
      :Nhận danh sách xếp hạng mới kèm lý do đề xuất;
    else ([Không])
      :Dự phòng: Lấy trực tiếp 10 kết quả đầu tiên từ pgvector;
    endif
  endif
...
Gemini --> GR : Trả về top 10 sân tốt nhất kèm lý do tiếng Việt
...
participant "RecService" as Service
database "Redis" as Redis
database "Database" as PG
participant "Gemini API" as LLM

User -> Client : Access homepage / view suggestions
||40||
Client -> Service : GET /v1/recommendations
activate Service
||40||

Service -> PG : Query Player status
activate PG
||40||
...
||40||
Service -> Redis : GET rec:player:id
activate Redis
||40||
...
||40||
  loop Wait & retry (max 3 times)
||40||
    Service -> Redis : SET lock:rec:id NX EX 15
    activate Redis
||40||
...
||40||
    ...
    alt lockAcquired == true
||40||
      note over Service : Proceed to calculation
    else lockAcquired == false
||40||
      Service -> Redis : GET rec:player:id
      activate Redis
||40||
      ...
      deactivate Redis
||40||
      alt concurrentCachedResponse != null
||40||
        Service --> Client : Success (200)
        deactivate Service
      end
...
||40||
  Service -> PG : Query completed bookings (limit 30)
  activate PG
||40||
...
||40||
    ...
    deactivate Redis
||40||
    alt cachedPopular == null
||40||
      Service -> PG : Query top fields by rating
      activate PG
||40||
      PG --> Service : popularSubfields
      deactivate PG
||40||
      Service -> Redis : SET rec:popular EX 3600
    end
||40||
    Service -> Redis : SET rec:player:id EX 3600
||40||
    Service -> Redis : DEL lock:rec:id
||40||
    Service --> Client : Success (200)
    deactivate Service
    
  else sampleSize >= 3 (Personalized)
||40||
    Service -> PG : Query recent bookings
    activate PG
||40||
    PG --> Service : recentBookings
    deactivate PG
||40||

    == Bước 4: Tìm kiếm tương đồng véc-tơ (pgvector) ==
||40||
    Service -> PG : Query similar fields (pgvector)
    activate PG
||40||
    PG --> Service : similarSubfields
    deactivate PG
||40||

    Service -> PG : Hydrate candidates details
    activate PG
||40||
...
||40||
    ...
    activate LLM
||40||
    alt Gemini success (<12s)
||40||
      LLM --> Service : JSON (Top 10 with reasons)
    else Gemini error / Timeout
||40||
      note over Service : Trigger fallback (use pgvector results)
    end
    deactivate LLM
||40||

    Service -> Service : Hydrate response
||40||
    Service -> Redis : SET rec:player:id EX 21600
||40||
    Service -> Redis : DEL lock:rec:id
||40||
    Service --> Client : Success (200)
...
skinparam ParticipantBorderThickness 3
skinparam ArrowThickness 2
<style>
sequenceDiagram {
  lifeLine {
...
BE -> DB: Get bookings & Stripe Account
activate DB
||40||
DB --> BE: Bookings & Stripe status
deactivate DB
||40||

alt stripe_onboarding == false
||40||
  BE --> FE: Error (400)
  deactivate BE
||40||
  FE --> P: 400 Bad Request
  
else stripe_onboarding == true
||40||
  BE -> DB: Extend pending bookings
||40||
  BE -> Stripe: stripe.checkout.sessions.create()
  activate Stripe
||40||
  Stripe --> BE: Checkout Session URL & ID
  deactivate Stripe
||40||
  BE --> FE: Success (200)
  deactivate BE
||40||
  FE -> P: Redirect to\nStripe Checkout
||40||
  P -> Stripe: Enter card & confirm payment
  activate Stripe
||40||
  Stripe -> Stripe: Process payment
||40||
  Stripe --> P: Complete & redirect to success_url
  
  == Xử lý sự kiện hoàn tất (Webhook) ==
||40||
  Stripe -> BE: Webhook: POST /webhooks/stripe\n(checkout.session.completed)
  deactivate Stripe
  activate BE
||40||
  BE -> DB: Verify signature & check transaction
  activate DB
||40||
  DB --> BE: Valid (unprocessed)
  deactivate DB
||40||
  BE -> DB: Update Payment (SUCCESS)\n& Bookings (COMPLETED)
  activate DB
||40||
  DB --> BE: Success (Commit)
  deactivate DB
||40||
  BE -> BE: Send notifications to users
||40||
  BE --> Stripe: Success (200)
  deactivate BE
||40||
end
@enduml
...
P -> FE: Select\nVNPay payment
||40||
FE -> BE: POST /payments/vnpay\n/checkout-session
activate BE


BE -> DB: create Payment (FAILED)\n& lock bookings
activate DB
||40||
DB --> BE: Success (Payment & Bookings)
deactivate DB
||40||

BE -> BE: Generate VNPay payment URL

BE --> FE: Success (200)
deactivate BE
||40||

FE -> P: Redirect to VNPay
||40||
P -> VNPay: Process payment
activate VNPay
...
||40||
Admin -> DB: Get PENDING OwnerPayouts
activate DB
||40||
DB --> Admin: OwnerPayouts list
deactivate DB


Admin -> Admin: Transfer 90% \nrevenue to Owner


Admin -> DB: Update OwnerPayout (COMPLETED)
activate DB
||40||
DB --> Admin: Success
deactivate DB
...
[2] S. Sanfilippo, "Distributed locks with Redis", *Redis Documentation*. [Online]. Available: https://redis.io/docs/latest/develop/use/patterns/distributed-locks/.

[3] PostgreSQL Global Development Group, "Explicit Locking", *PostgreSQL 16 Documentation*. [Online]. Available: https://www.postgresql.org/docs/16/explicit-locking.html.

[4] P. Covington, J. Adams, and E. Sargin, "Deep Neural Networks for YouTube Recommendations", in *Proceedings of the 10th ACM Conference on Recommender Systems*, Boston, Massachusetts, 2016, pp. 191–197.

[5] pgvector Contributors, "pgvector: Open-source vector similarity search for PostgreSQL", *GitHub Repository*. [Online]. Available: https://github.com/pgvector/pgvector.

[6] J. Zhu et al., "Large Language Models for Information Retrieval: A Survey", *arXiv preprint arXiv:2308.07107*, 2023.

[7] Google Cloud, "Structured Outputs", *Gemini API Documentation*. [Online]. Available: https://ai.google.dev/gemini-api/docs/structured-outputs.
