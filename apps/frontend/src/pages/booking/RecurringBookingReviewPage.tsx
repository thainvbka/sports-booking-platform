...
import { bookingService, type RecurringBookingReviewResponse } from "@/services/booking.service";
import { formatPrice } from "@/services/mockData";
import { Button } from "@/components/ui/button";
import { Card, CardContent, CardHeader, CardTitle } from "@/components/ui/card";
import { Separator } from "@/components/ui/separator";
import { Calendar, Clock, MapPin, Trophy, Repeat } from "lucide-react";
...
export default function RecurringBookingReviewPage() {
  ...
  const [booking, setBooking] = useState<RecurringBookingReviewResponse | null>(null);
  ...
  if (!booking) return null;
    ...
    const startTimeStr = firstSlot ? format(new Date(firstSlot.date), "HH:mm") : "";
  ...
  return (
    <div className="container mx-auto px-4 py-8 max-w-4xl">
      <Card>
        <CardHeader>
          <CardTitle className="text-2xl font-bold text-center">Xác nhận đặt sân định kỳ</CardTitle>
        </CardHeader>
        <CardContent className="grid gap-6 md:grid-cols-2">
            <div className="space-y-6">
                ...
                <div className="space-y-2">
                 ...
                 <div className="space-y-4">
                    <div className="flex items-center gap-3">
                     ...
                     <div className="flex items-center gap-3">
                        ...
                        <p className="font-medium">
                             {booking.recurrence_type === "WEEKLY" ? "Hàng tuần" : "Hàng tháng"}
                        </p>
                        </div>
                    </div>
                 ...
                 <Button className="w-full text-lg py-6 mt-2" onClick={handlePayment}>
                    Thanh toán ngay
                </Button>
            </div>
            ...
            <div className="border rounded-lg p-4">
                <h4 className="font-semibold mb-4 flex items-center gap-2">
                    <Clock className="w-4 h-4" /> Danh sách các buổi đá
                </h4>
                <ScrollArea className="h-[400px] w-full pr-4">
                    <div className="space-y-3">
                        {booking.slots.map((slot, index) => (
                            <div key={index} className="flex justify-between items-center p-3 bg-muted/50 rounded-md text-sm">
                                <span className="font-medium">
                                     {format(new Date(slot.date), "EEEE, dd/MM/yyyy - HH:mm", { locale: vi })}
                                </span>
